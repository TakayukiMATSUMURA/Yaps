...
public:
    /// Default constructor
    MULT () : RFormat() {}
    MULT (const BinaryData& binary) : RFormat(binary) {}
    
    /// Destructor
    virtual ~MULT () {};

    void execute () override {
        result64bit_ = rs_.signedValue() * rt_.signedValue();
    }
    
    std::string mnemonic () const override {return "mult";}
    int opcode () const override {return ISA::SPECIAL;}
    int funct () const override {return ISA::MULT;}
...
auto registerMult = Decoder::instance()->registerInst<MULT>();
